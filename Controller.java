    ...
    public void ClickToLog(ActionEvent actionEvent) {
        try {
         ...
         Platform.runLater(() -> {
             ...
             String color_style1 = c2.toString().substring(2, 8);
             Node node1 = lineChart.lookup(".default-color0.chart-series-line");
             node1.setStyle("-fx-stroke: #" + color_style1 + ";");
             Node legend1 = lineChart.lookup(".default-color0.chart-legend-item-symbol");
             if (legend1 != null) {
                 legend1.setStyle("-fx-background-color: #" + color_style1 + ", white;");}
             for (int i = 0; i < series2.getData().size(); i++) {
                 XYChart.Data<Double,Double> round1 = series2.getData().get(i);
                 Node roundColor1 = round1.getNode().lookup(".chart-line-symbol");
                 roundColor1.setStyle("-fx-background-color: #" + color_style1 + "; -fx-background-radius: 0; -fx-padding: 1px;");
             }
         });
    }
    @FXML
